...
import { ArrowRight, Shield, IndianRupee, Zap, CheckCircle } from "lucide-react";
...
/* Image pool — real self-employed customers across TN / AP / TG.
   Photos: Wikimedia Commons, CC BY/BY-SA — see /credits.txt. Amounts indicative (~₹7L avg). */
const pool = [
...
const Tile = ({ idx, className }: { idx: number; className: string }) => {
  ...
  return (
    <div className={`relative overflow-hidden rounded-3xl shadow-clay ring-1 ring-foreground/5 ${className}`}>
      ...
      {/* Consistent warm overlay to unify the heterogeneous photos */}
...
const HeroSectionV2 = () => {
  // Three tile slots, each pointing at a pool index; they advance staggered.
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      setSlots((prev) => {
        const next = [...prev];
        // jump this slot forward by 3 so it lands on an image not currently shown
        next[slot] = (next[slot] + 3) % pool.length;
        return next;
      });
      tick++;
    }, 1600); // each individual tile changes ~every 4.8s
  ...
  return (
    <section className="relative min-h-[88vh] flex items-stretch overflow-hidden bg-hero">
      ...
      <div className="flex-1 pl-5 md:pl-[max(1.25rem,calc((100vw-1280px)/2+1.25rem))] pr-5 md:pr-12 py-20 md:py-24 flex flex-col justify-center relative z-10">
        ...
        <motion.div custom={3} initial="hidden" animate="visible" variants={fadeUp}
          ...
          <Button variant="hero" size="lg" className="text-sm animate-cta-glow" asChild>
            <Link to="/apply">Apply for a Loan <ArrowRight size={15} /></Link>
          </Button>
      ...
      {/* ── Right — controlled rotating collage (desktop only) ── */}
      <div className="hidden lg:flex items-center justify-center w-[46%] shrink-0 self-stretch pl-4 pr-10 xl:pr-16 py-20 relative z-10">
        <div className="relative w-full max-w-[460px]">
          ...
          {/* Indicative disbursement badge (follows featured tile) */}
          <motion.div
