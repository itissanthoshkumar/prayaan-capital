      ...
      <section className="pt-24 pb-8 md:pt-32 md:pb-12 bg-hero relative overflow-hidden">
        ...
        <div className="container mx-auto px-5 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="font-display text-3xl md:text-6xl font-bold text-foreground mt-4 mb-3">Foreclosure Calculator</h1>
          ...
          <div className="clay-surface overflow-hidden">
            ...
            <div className="grid md:grid-cols-2">
              <div className="p-6 md:p-10 space-y-8 border-r border-border/40">
                <div>
                  <div className="flex justify-between mb-3"><label className="text-sm font-semibold">Outstanding Principal</label><span className="font-mono font-bold text-primary">₹{outstanding}L</span></div>
                  <Slider aria-label="Outstanding principal in lakhs" value={[outstanding]} onValueChange={(v) => setOutstanding(v[0])} min={1} max={50} step={1} />
                </div>
                <div>
                  <div className="flex justify-between mb-3"><label className="text-sm font-semibold">EMIs Already Paid</label><span className="font-mono font-bold text-accent">{paid}</span></div>
                  <Slider aria-label="EMIs already paid" value={[paid]} onValueChange={(v) => setPaid(v[0])} min={1} max={36} step={1} />
                </div>
                <div>
              ...
              <div className="p-6 md:p-10 bg-muted/20 space-y-5">
                <div><p className="text-xs uppercase text-muted-foreground tracking-wider">Foreclosure charges</p><p className="text-3xl font-mono font-bold text-foreground mt-1">{fmt(charges)}</p></div>
                <div><p className="text-xs uppercase text-muted-foreground tracking-wider">Total payable today</p><p className="text-3xl font-mono font-bold text-primary mt-1">{fmt(payable)}</p></div>
                <div><p className="text-xs uppercase text-muted-foreground tracking-wider">Interest you save</p><p className="text-3xl font-mono font-bold text-accent mt-1">{fmt(savings)}</p></div>
