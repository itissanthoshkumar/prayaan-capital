...
const EligibilityCalc = () => {
  const [propertyValue, setPropertyValue] = useState(60); // ₹L
  const [income, setIncome] = useState(80);               // ₹K / month
  ...
  const { eligible, amount, score } = useMemo(() => {
    ...
    const eligibleAmt = Math.min(100, Math.max(10, Math.round(Math.min(ltvCap, incomeCap))));
          ...
          <div className="clay-surface overflow-hidden">
            ...
            <div className="grid md:grid-cols-2">
              <div className="p-6 md:p-10 space-y-8 border-r border-border/40">
                <div>
                  <div className="flex justify-between mb-3"><label className="text-sm font-semibold">Property Value</label><span className="font-mono font-bold text-primary">₹{propertyValue}L</span></div>
                  <Slider aria-label="Property value in lakhs" value={[propertyValue]} onValueChange={(v) => setPropertyValue(v[0])} min={10} max={500} step={5} />
                </div>
                <div>
                  <div className="flex justify-between mb-3"><label className="text-sm font-semibold">Monthly Income</label><span className="font-mono font-bold text-accent">₹{income}K</span></div>
                  <Slider aria-label="Monthly income in thousands" value={[income]} onValueChange={(v) => setIncome(v[0])} min={20} max={500} step={5} />
                </div>
                <div>
                  <div className="flex justify-between mb-3"><label className="text-sm font-semibold">CIBIL Score</label><span className="font-mono font-bold text-[hsl(var(--color-lavender))]">{cibil}</span></div>
                  <Slider aria-label="CIBIL score" value={[cibil]} onValueChange={(v) => setCibil(v[0])} min={550} max={850} step={5} />
                </div>
              </div>
              <div className="p-6 md:p-10 bg-muted/20 flex flex-col justify-center">
                ...
                <div className="text-5xl font-mono font-bold text-primary mb-4">{score}<span className="text-2xl text-muted-foreground">/100</span></div>
